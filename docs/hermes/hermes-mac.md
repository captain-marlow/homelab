...
- **Stopping Hermes.** `hermes gateway stop` cleanly unloads the LaunchAgent and keeps it down
  ...
  call `launchctl bootout` internally, not just kill the process). **Real status check:**
  `launchctl list | grep hermes` — not `hermes gateway status` alone. `hermes gateway uninstall`
  removes the LaunchAgent entirely. Use `hermes gateway restart` only to *reload* a running
  gateway after `.env` edits.

...
- **Not canonical executor yet:** Phase 3 resumes at Step 3 (`github-omega` write key + omega proves own commit+push), then Step 4 (one infra action, verified live) and Step 5 (parity) before Phase 4 cutover. Hermes remains the active read-write executor until cutover. Cleanup: omega's `omega_homelab_ed25519` is currently authorized as root on pve01; scope to least privilege when convenient.

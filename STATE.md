...
- **Per-agent model chains (updated 2026-06-27):** model chains are now configured per-agent in `agents.list[].model`, not as a single shared chain. `agents.defaults.model` (`anthropic/claude-opus-4-8` → `anthropic/claude-sonnet-4-6`) is only the inherited base for unconfigured agents/surfaces.

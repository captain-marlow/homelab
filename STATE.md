...
- **Repo write topology (2026-07-01):** omega is the **sole git writer** (`github-omega`). The CT175 clone is **read-only** (`github-homelab` key), shared by `@architect` + `@openclaw` as a pull-only mirror — CT175 agents never commit (local commits orphan; see `e87bf4b`/`9d38686`). All repo writes route to omega, pull-before-act. Full statement: `docs/openclaw/two-agent-loop.md` §Repo write topology.

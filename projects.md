...
| D007 | Agent config versioning + backup (architect, OpenClaw, omega) | agents | idea | Two halves: (1) **version-control** sanitized identity/config in `agents/<name>/` — architect already there; add `agents/openclaw/` (SOUL + sanitized `openclaw.json`) and `agents/omega/` (SOUL + sanitized `config.yaml`/`.env.example`), secrets externalized via SecretRef/env (never in repo). Hermes config is dormant/rollback — not an active versioning target. (2) **back up the secrets** — `~/.homelab-secrets` (Mac), CT175 `openclaw.json` secrets, CT171 `/config/.env`, omega `.env`/secrets + omega Matrix recovery key (Hermes creds retained only as dormant rollback) — to an encrypted off-box store + password manager, with a documented restore. The real gap: no systematic backup of live settings or of `~/.homelab-secrets` itself. Pairs with D001/D002 (rotation) and the agents/ layout. |
...
| D010 | omega Matrix history-backfill (passive room buffer) — passive room context analog | openclaw/matrix | deferred | **DECIDED 2026-06-28: option (c) — do NOT build the passive-room buffer now.** omega stays addressed-only; pill it with context when needed. Keep **(b) upstream-PR to Nous** noted as the preferred future path over a local fork. If ever built, size the buffer to **50** to match the others. |
...
| P006 | Hermes on Mac — standalone + repo integration + Matrix loop (`@hermes` live in Drafting Table, E2EE, mention-gated; Mac-side read-write/git-push executor) | hermes | done | `docs/hermes/hermes-mac.md` — Superseded by omega (D012, 2026-07-01). |
...
- **Hermes (P006) is sequenced before the docs work (P007 → P008):** P008's "apply + push the styled rewrites" step needs the central read-write executor, which was Hermes, now omega (the architect is pull-only; OpenClaw write access is deferred). So Hermes must exist before the sort pass. IDs are non-sequential here (P006 = Hermes predates the docs projects); order lives in the queue line.
...
- P008 reuses the two-agent loop (architect proposes styled rewrites read-only → human gate → **omega** applies + pushes) — documentation cleanup is itself a planner→executor task.
- **Git writers (D012 done):** Claude Code and **omega** both write to the Mac-side repo now (distinguish by commit author). With omega's dedicated identity + write deploy key (`github-omega`), repo mutations driven by the loop should be **delegated to omega** so authorship/role boundaries stay clear (`@openclaw` should no longer commit via Ryan's Mac clone). The architect's clone (CT175) stays a read-only mirror; refresh all three clones after every push.
